...
function taskUrl(turnTableId) {
  const url = `${JD_API_HOST}/api/turncard/channel/detail?turnTableId=${turnTableId}&invokeKey=`
  return {
    url,
    headers: {
...
function tasPostkUrl(turnTableId) {
  const url = `${JD_API_HOST}/api/turncard/channel/sign?turnTableId=${turnTableId}&fp=${fp}&eid=${eid}&invokeKey=`
  return {
    url,
    headers: {
      "Accept": "application/json, text/plain, */*",
