 ...
 * name: 京东-双11星推官
 * author: @lof
 * cron "13 0,12 * * *" script-path=jd_1111xtg.js, tag=v
 */
const $ = new Env("✨双11星推官");
const jdCookieNode = $["isNode"]() ? require("./jdCookie.js") : "";

let inviteCodes = [
    { user: '恋你莲我', shareId: 'cf4493b9-f308-4e14-8098-ea954abd29cf' },
    {user: 'jd_rtrqrVNXHIXE', shareId: '6b5bf432-abe4-45fe-8083-e0db4978e036'}
];
let cookiesArr = [],
  cookie = "";
...
if (process["env"]["XTG_1111_LOTTERY"])
  XTG_1111_LOTTERY = process["env"]["XTG_1111_LOTTERY"];
      ...
      if (cookiesArr[i]) {
        ...
        for (let i = 0; i < 8; i++) {
          ...
          for (let task of tasksList) {
            ...
            if (task.taskState == "0") {
              console.log(
                `任务${task.taskName},parentId=${task.parentId}taskId=${task.taskId}【已完成】`
              );
            } else {
              console.log(
                `任务${task.taskName},parentId=${task.parentId}taskId=${task.taskId}【做任务】`
              );
              startT = new Date().getTime();
              rewardST = await xtg_do_task(task.parentId, task.taskId, startT);
              if (task.type == "BROWSE_TASK" || rewardST) {
                await $.wait(1000 * 12);
                await xtg_get_reward(
                  task.parentId,
                  task.taskId,
                  rewardST,
                  new Date().getTime()
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
      ...
      for (let code of inviteCodes) {
        ...
        if (code["shareId"] && code["shareId"] != "undefined") {
          console.log(
            `【${$.UserName}】去助力【${code["user"]}】邀请码：${code["shareId"]}`
          );
          if (!$.canHelp) break;
...
function xtg_inner_help(inviteId) {
  ...
  let opt = {
    ...
    body: `appid=china-joy&functionId=star_push_jd_prod&body=${escape(
      JSON.stringify(body)
    )}&t=${new Date().getTime()}&loginType=2`,
    headers: {
      Cookie: cookie,
      Host: "api.m.jd.com",
        ...
        if (err) {
          ...
          if (data.code === 200) {
            ...
            if (data.data && data.data.status === 4)
              console.log(`助力结果：对方助力已满`);
            if (data.data && data.data.status === 5)
              console.log(`助力结果：助力次数不足`);
            $.canHelp = false;
          ...
          if (safeGet(data)) {
            ...
            if (data.code === 200) {
              ...
              if (data.data.prizeName) {
                console.log(
                  `🎉恭喜获得${data.data.prizeName} ${data.data.prizeCount} 个`
                );
              } else {
                console.log(`未中奖`);
              }
