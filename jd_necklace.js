...
================Loon==============
[Script]
cron "49 0,6,13,20 * * *" script-path=jd_necklace_new.js,tag=点点券
===============Surge=================
点点券 = type=cron,cronexp="49 0,6,13,20 * * *",wake-system=1,timeout=3600,script-path=jd_necklace_new.js
============小火箭=========
点点券 = type=cron,script-path=jd_necklace_new.js, cronexpr="49 0,6,13,20 * * *", timeout=3600, enable=true
 */
const $ = new Env('点点券_全任务版');
...
function showMsg() {
  return new Promise(async resolve => {
    ...
    //}
    console.log(`${$.name} - 京东账号${$.index} - ${$.nickName}`, `京东账号${$.index} ${$.nickName}\n当前${$.name}：${$.totalScore}个\n可兑换无门槛红包：${$.totalScore / 1000}元\n\n`);
    resolve()
  })
}
