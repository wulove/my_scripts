...
const uuid = 'fc13275e23b2613e6aae772533ca6f349d2e0a86';
...
async function doTask() {
  for (let item of $.taskConfigVos) {
    console.log(item.taskStage, item['taskType'], item.taskName)
    if (item.taskStage === 0) {
...
async function reportTask(item = {}) {
  ...
  //逛很多商品店铺等等任务
  if ([6, 8, 5, 9].includes(item['taskType'])) {
    //浏览精选活动任务
    ...
    for (let vo of $.taskItems) {
      ...
      await necklace_reportTask(item.id, vo['id']);
    }
  }
  //首页浏览XX秒的任务
          ...
          if (safeGet(data)) {
            ...
            if (data.rtn_code === 0) {
              if (data.data.biz_code === 0) {
                console.log(`签到成功，时间：${new Date().toLocaleString()}`)
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.rtn_code === 0) {
              if (data.data.biz_code === 0) {
                console.log('领取点点券成功！获得', data.data.result.giftScoreNum)
                // $.taskConfigVos = data.data.result.taskConfigVos;
                // $.exchangeGiftConfigs = data.data.result.exchangeGiftConfigs;
              }
            } else {
...
function necklace_reportTask(taskId, itemId = "") {
...
function necklace_startTask(body) {
    ...
    $.post(taskPostUrl('necklace_homePage'), async (err, resp, data) => {
      try {
        if (err) {
          ...
          data = JSON.parse(data);
          if (data.rtn_code === 0 && data.data.biz_code === 0) {
            let result = data.data.result;
            $.taskConfigVos = result.taskConfigVos;
            $.exchangeGiftConfigs = result.exchangeGiftConfigs;
...
function taskPostUrl(function_id, body = {}) {
  return {
    url: `${JD_API_HOST}?functionId=${function_id}&appid=coupon-necklace&loginType=2&client=coupon-necklace&t=${Date.now()}&body=${escape(JSON.stringify(body))}&uuid=${uuid}`,
    headers: {
      'Host': 'api.m.jd.com',
      'accept': 'application/json, text/plain, */*',
      'content-type': 'application/x-www-form-urlencoded',
...
function requireConfig() {
  return new Promise(resolve => {
    ...
    if ($.isNode()) {
      ...
      cookiesArr = [$.getdata('CookieJD'), $.getdata('CookieJD2'), ...JSON.parse($.getdata('CookiesJD') || "[]").map(item => item.cookie)].filter(item => !!item);
    }
