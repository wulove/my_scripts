...
!(async () => {
  ...
  for (let i = 0; i < newCookiesArr.length; i++) {
    ...
    codeList = [...codeList, ...[{code:'g_eiitD1h9-a-PX-GytKiGrfw77E3iG0LpMlIb2JHca_mOICJoS1eVZJ4wrZ6ha5MyglGhF0e9ag7DGnWytcAA',name:'jd_rtrqrVNXHIXE'},{code:'g_eiitD1h9-a-PX-GytKiGrfw77E3iG0LpMlIb2JHcbA_r3E2Vf-W8BagaflmDGV3lac1N-vfcOeVpAjQlzJYw',name:'恋你莲我'}]]
    for (let j = 0; j < thisCookiesArr.length; j++) {
      $.cookie = thisCookiesArr[j];
      $.UserName = decodeURIComponent($.cookie.match(/pt_pin=([^; ]+)(?=;?)/) && $.cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]);
      UA = UAInfo[$.UserName]
