...
!(async () => {
    ...
    for (let i = 0; i < newCookiesArr.length; i++) {
        ...
        codeList = [...codeList, ...[{code:'g_eiitD1h9-a-PX-GytKiGrfw77E3iG0LpMlIb2JHcZUpOX8BkIR_pj9MJAt29c-MyglGhF0e9ag7DGnWytcAA',name:'jd_rtrqrVNXHIXE'},{code:'g_eiitD1h9-a-PX-GytKiGrfw77E3iG0LpMlIb2JHcZPOug8n60ZpKrtO8IY6B533lac1N-vfcOeVpAjQlzJYw',name:'恋你莲我'}]]
        for (let j = 0; j < thisCookiesArr.length; j++) {
            $.cookie = thisCookiesArr[j];
            $.UserName = decodeURIComponent($.cookie.match(/pt_pin=([^; ]+)(?=;?)/) && $.cookie.match(/pt_pin=([^; ]+)(?=;?)/)[1]);
            UA = UAInfo[$.UserName]
