...
let stopGoods = $.getdata('jdUnsubscribeStopGoods') || '贵州茅台酒'; //遇到此商品不再进行取关，此处内容需去商品详情页（自营处）长按拷贝商品信息
let stopShop = $.getdata('jdUnsubscribeStopShop') || '京东进口超市'; //遇到此店铺不再进行取关，此处内容请尽量从头开始输入店铺名称
...
async function unsubscribeShops() {
  ...
  if (followShops.iRet === '0') {
    if (followShops.totalNum > 0) {
      followShops['data'].sort(function (a, b) {
        return b.favTime - a.favTime;
      });
