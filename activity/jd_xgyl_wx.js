...
更新地址：https://github.com/wulove/my_scripts/raw/master/jd_xgyl_wx.js
...
[task_local]
#小鸽有礼
30 7 * * * https://github.com/wulove/my_scripts/raw/master/jd_xgyl_wx.js, tag=小鸽有礼,  enabled=true

================Loon==============
[Script]
cron "30 7 * * *" script-path=https://github.com/wulove/my_scripts/raw/master/jd_xgyl_wx.js, tag=小鸽有礼

===============Surge=================
小鸽有礼 = type=cron,cronexp="30 7 * * *",wake-system=1,timeout=3600,script-path=https://github.com/wulove/my_scripts/raw/master/jd_xgyl_wx.js

============小火箭=========
小鸽有礼 = type=cron,script-path=https://github.com/wulove/my_scripts/raw/master/jd_xgyl_wx.js, cronexpr="30 7 * * *", timeout=3600, enable=true
 */
const $ = new Env('小鸽有礼');
...
function getMissionList() {
  return new Promise(resolve => {
    $.post(taskUrl('WonderfulLuckDrawApi/queryMissionList'), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(resp)
...
function getDrawChance(body) {
  return new Promise(resolve => {
    $.post(taskUrl('WonderfulLuckDrawApi/getDrawChance', body), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(resp)
...
function doMission(body) {
  return new Promise(resolve => {
    $.post(taskUrl('WonderfulLuckDrawApi/completeMission', body), (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(resp)
...
function taskUrl(function_id, body) {
  return {
    ...
    headers: {
      ...
      'jexpress-trace-id': '5rkvle8bdo8bdyj3',
      'x-requested-with': 'XMLHttpRequest',
      'source-client': '1',
      'AppParams': '{"appid":158,"ticket_type":"m"}',
      'version': '1619427826000',
      'event-id': 'bgwlzg4xl9z6vkop',
      'referer': 'https://servicewechat.com/wxc39d4a8602c12fda/6/page-frame.html',
